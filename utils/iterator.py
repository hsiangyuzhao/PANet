...
class LRFinder:
    def __init__(self, model, device, train_loader, criterion, optimizer):
        self.model = model
        self.device = device
    ...
    def find_lr(self, init_value=1e-8, final_value=10., beta=0.98):
        ...
        for batch_num, batch_data in enumerate(self.train_loader):
            ...
            inputs_t1 = batch_data['t1'][tio.DATA].to(self.device)
            inputs_t2 = batch_data['t2'][tio.DATA].to(self.device)
            inputs_t1ce = batch_data['t1ce'][tio.DATA].to(self.device)
            inputs_flair = batch_data['flair'][tio.DATA].to(self.device)
            inputs = torch.cat([inputs_t1, inputs_t2, inputs_t1ce, inputs_flair], dim=1)

            # load targets
            targets = batch_data['label'][tio.DATA].to(self.device)
