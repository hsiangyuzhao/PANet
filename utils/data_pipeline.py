...
class InferenceReader:
    ...
    def get_dataset(self, test_size, random_state):
        print('Prepare val dataset for cross-validation. Test size: {}; Random state: {}'.format(test_size, random_state))
        preprocess = self.get_preprocessing_transform()
        _, val_subject_list = train_test_split(self.subject_list,
                                                                test_size=test_size,
                                                                random_state=random_state)
        print('Val subjects: {}'.format(val_subject_list))
...
def discretize_labels(target_tensor):
    ...
    :param target_tensor: shape (num_channel, H, W, D)
    :return:
    """
